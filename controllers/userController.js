...
router.get("/", verifyToken, (req, res) => {
  User.find()
    .select("-password -notificationTokens -notifications")
    .then(users => {
      res.json(users);
    });
});
...
router.post("/reset", (req, res) => {
  ...
  User.findOne(query)
    .then(user => {
      ...
      if (
        user.info.email !== undefined &&
        user.info.email !== "" &&
        user.info.email.toLowerCase() !== body.email.toLowerCase()
      ) {
...
// Creates a user
router.post("/", (req, res) => {
  GoldenTicket.verifyTicket(req.body.goldenTicket)
    .then(ticket => {
      if (ticket) {
...
router.post("/:id/doninfo", verifyToken, (req, res) => {
  if (!req.user.role || !req.user.role.includes("don")) {
    console.error(
      `User ${req.user._id}is requesting don info when user is not a don`
    );
...
router.put(
  ...
  (req, res) => {
    ...
    User.findOne({ _id: req.params.id })
      ...
      .then(user => {
        user
          .updateProfilePicture(req.file.buffer)
          .then(pic => {
...
router.post("/:id/notificationToken", verifyToken, (req, res) => {
  ...
  User.findOne({ _id: req.params.id })
    .then(user => {
      user
        .registerNotificationToken(req.body.notificationToken)
        .then(token => {
          res.json(token);
        })
        .catch(err => {
