...
const authServerAddress =
  process.env.AUTH_SERVER_URI || "https://skybunk-auth-dev.herokuapp.com";
...
module.exports.generateTickets = count =>
  fetch(`${authServerAddress}/servers/${process.env.SERVER_ID}/tickets`, {
    method: "POST",
    headers: {
    ...
    .then(async jsonResponse => {
      await GoldenTicket.insertMany(
        jsonResponse.map(ticketNumber => ({ ticketNumber }))
      );
