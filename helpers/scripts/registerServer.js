...
const getAuthUrl = () => promptForText('Enter auth server url:', DEFAULT_AUTH_SERVER_URL);

const getServerName = () => promptForText('Enter new server name:', DEFAULT_SERVER_NAME);
...
const getServerUrl = async (authUrl, existingServers) => {
    ...
    if (!tunnel) {
        ...
        console.log('You can start a tunnel with the following command if desired:');
...
(async () => {
    ...
    const {_id, accessKey, errmsg} = await registerServer({authUrl, name, url});
    if (errmsg) {
        console.error(errmsg);
        return;
    }
    console.log(`Server (${name}) registered with credentials (DON'T LOSE THEM):`);
