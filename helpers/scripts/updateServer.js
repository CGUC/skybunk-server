...
(async () => {
    ...
    if (!name) {
        console.error(`Server with id ${process.env.SERVER_ID} not found. Is SERVER_ID correct?`);
        return;
    }
    ...
    if (await promptForConfirmation('Change server url?', true)) {
        const newUrl = await getServerUrl(process.env.AUTH_SERVER_URI, servers.map(({url}) => url));
        if (newUrl !== url) updates.url = newUrl;
    }

    if (updates) {
        console.log('Updating server information.');
        await updateServer(process.env.AUTH_SERVER_URI, process.env.SERVER_ID, process.env.ACCESS_KEY, updates);
        console.log('Done!');
    } else {
