...
const buildAdminUser = async () => {
  ...
  try {
    ...
    console.error(
      `An auth server does not appear to be running at '${authServerAddress}'`
    );
    return;
  }
  ...
  const password = await promptForText({
    type: "password",
    message: "Enter password"
  });
  const firstName = await promptForText({
    message: "Enter first name:",
    defaultValue: "Admin"
  });
  const lastName = await promptForText({
...
(async () => {
  ...
  const { errmsg } = await createUserOnAuth({
    ...adminUser,
