...
PostSchema.statics.create = function(postData, author) {
  ...
  return new Promise((resolve, reject) => {
    if (post.content.length > config.postCharacterLimit) {
      reject(
        Error(
          `Post cannot be more than ${config.postCharacterLimit} characters`
        )
      );
    }
...
PostSchema.statics.updatePost = function(id, postData) {
  ...
  return new Promise((resolve, reject) => {
    if (
      postData.content &&
      postData.content.length > config.postCharacterLimit
    ) {
      reject(
        Error(
          `Post cannot be more than ${config.postCharacterLimit} characters`
    ...
    this.findById(id).then(post => {
      if (post) {
        const updatedPost = _.extend(
          post,
          _.pick(postData, ["content", "tags"])
        );
    ...
    this.findById(id).then(post => {
      if (post && post.usersLiked) {
        if (addLike) {
          ...
          post.usersLiked.some(e => e.toString() === user.toString())
        ) {
...
PostSchema.statics.addComment = function(id, commentData, author) {
  ...
  return new Promise((resolve, reject) => {
    if (comment.content.length > config.commentCharacterLimit) {
      reject(
        Error(
          `Comment cannot be more than ${config.commentCharacterLimit} characters`
        )
      );
    }
    ...
    this.get(id)
      .then(post => {
        ...
        const userAlreadySubscribed = post.subscribedUsers.some(
          user => user._id.toString() === comment.author.toString()
        );
          ...
          .then(updatedPost => {
            ...
            post.subscribedUsers.forEach(user => {
              ...
              if (user._id.toString() !== comment.author.toString()) {
                ...
                NotificationManager.saveNotificationForUserAsync(
                  notificationData,
                  user
                );
...
PostSchema.statics.updateComment = function(postId, commentId, commentData) {
  ...
  return new Promise((resolve, reject) => {
    if (
      comment.content &&
      comment.content.length > config.commentCharacterLimit
    ) {
      reject(
        Error(
          `Comment cannot be more than ${config.commentCharacterLimit} characters`
...
PostSchema.statics.likeComment = function(postId, commentId, user, addLike) {
  ...
  return this.get(postId).then(post => {
    ...
    return new Promise((resolve, reject) => {
      if (addLike) {
        if (
          comment.usersLiked.some(e => e._id.toString() === user._id.toString())
        ) {
          reject(Error("Already liked"));
        } else {
          // add user to list
          ...
          _comments: {
            past_24h: {
              $size: {
                $filter: {
                  input: "$comments",
                  cond: { $gte: ["$$this.createdAt", past24h] }
                }
              }
            },
            past_3d: {
              $size: {
                $filter: {
                  input: "$comments",
                  cond: { $gte: ["$$this.createdAt", past3d] }
                }
              }
            },
            past_7d: {
              $size: {
                $filter: {
                  input: "$comments",
                  cond: { $gte: ["$$this.createdAt", past7d] }
                }
              }
            },
            past_30d: {
              $size: {
                $filter: {
                  input: "$comments",
                  cond: { $gte: ["$$this.createdAt", past30d] }
                }
              }
            }
          }
    ...
    this.aggregate([
      ...
      },
      {
        $project: {
          author: 1,
          comments: {
            $cond: { if: "$_is_post", then: "not_a_comment", else: "$comments" }
          },
          createdAt: 1,
          _is_post: 1
        }
