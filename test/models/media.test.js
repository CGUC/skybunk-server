...
describe("Media", () => {
  ...
  describe("correctly validates", () => {
    it("if media type does not include proper media", done => {
      ...
      media.validate(err => {
        expect(err.errors.type.message).to.equal(
          "Type is set to image, yet no image provided."
        );
        done();
      });
    });
    it("if non-image type includes an image", done => {
      ...
      media.validate(err => {
        expect(err.errors.image.message).to.equal(
          "Type is not set to image, yet image was provided."
        );
        done();
      });
    });
  ...
  describe("can handle", () => {
    it("images successfully", done => {
      Media.create("image", buffer).then(result => {
        ...
        expect(result.image.buffer.toString("base64")).to.equal(
